...
var Otr = function(){
  ...
  const prehash_bytes = {
    'ssid': new Buffer('00', 'hex').toString('binary'),
    'c': new Buffer('01', 'hex').toString('binary'),
    'm1': new Buffer('02', 'hex').toString('binary'),
    'm2': new Buffer('03', 'hex').toString('binary'),
    'm1_prime': new Buffer('04', 'hex').toString('binary'),
    'm2_prime': new Buffer('05', 'hex').toString('binary')
  };
  ...
  this.reveal_sig_parse = function(message){
    ...
    this.reveal_sig_hash['enc_sig_mac'] = reveal_sig_buff.slice(enc_sig_end_offset, reveal_sig_buff.length);
  }

  this.reveal_sig_process = function(){

    // decrypt and verify gxmpi
    ...
    // compute hashes
    var secbytes = this.s_size.toString('binary') + this.s.toString('binary'); 
    this.ssid = new Buffer(helpers.sha256(prehash_bytes['ssid']+secbytes), 'binary').slice(0,8);
    var c_whole = new Buffer(helpers.sha256(prehash_bytes['c']+secbytes), 'binary');
    this.c = c_whole.slice(0,16);
    this.c_prime = c_whole.slice(16,32);
    this.m1 = new Buffer(helpers.sha256(prehash_bytes['m1']+secbytes), 'binary');
    this.m2 = new Buffer(helpers.sha256(prehash_bytes['m2']+secbytes), 'binary');
    console.log(this.m2);
    this.m1_prime = new Buffer(helpers.sha256(prehash_bytes['m1_prime']+secbytes), 'binary');
    this.m2_prime = new Buffer(helpers.sha256(prehash_bytes['m2_prime']+secbytes), 'binary');

    // verify enc_sig_mac using m2 with sha256-hmac-160
    var hmac = crypto.createHmac('sha256', this.m2);
    hmac.update(this.reveal_sig_hash['enc_sig_size'].toString('binary')+this.reveal_sig_hash['enc_sig'].toString('binary'));
    var enc_sig_mac = new Buffer(hmac.digest(), 'binary').slice(0,20);
    if(this.reveal_sig_hash['enc_sig_mac'].toString() != enc_sig_mac.toString){
      return new Error("The HMAC for the encrypted signature could not be verified");
    }
  }
